...
export default function BudgetCoach({
  ...
  const getCatColor = (cat) => CAT_OPTIONS.find(c => c.id === cat)?.color || "#64748B";
  const getCatIcon = (cat) => CAT_OPTIONS.find(c => c.id === cat)?.icon || "category";

  const today = new Date();
  const todayLogs = (loggedSpending || []).filter(log => new Date(log.date).toDateString() === today.toDateString());

  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden", background: "transparent", fontFamily: "Inter, sans-serif" }}>
      {/* Header */}
      <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 112px" }}>
        ...
        <div style={{ background: "white", borderRadius: 28, padding: 20, boxShadow: "0 2px 10px rgba(0,0,0,0.03)", marginBottom: 24 }}>
          ...
          {[
            ...
            { label: "Daily Pool", amount: pool, color: "#F59E0B", pct: (pool / monthlyIncome) * 100 }
          ].map(row => (
            <div key={row.label} style={{ marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 4 }}>
                <span style={{ color: "#64748B", fontWeight: 500 }}>{row.label}</span>
                <span className="font-mono" style={{ color: "#0F172A", fontWeight: 700 }}>{currency} {fmt ? fmt(row.amount) : row.amount}</span>
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {todayLogs.map(log => (
              <div key={log.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "white", padding: "16px", borderRadius: 24, boxShadow: "0 2px 8px rgba(0,0,0,0.02)" }}>
                ...
                <span className="font-mono" style={{ fontSize: 15, fontWeight: 700, color: "#0F172A" }}>{currency} {fmt ? fmt(log.amount) : log.amount}</span>
              </div>
            ))}
          </div>
        )}
